function [Subspaces,Projs,out] = SubspaceSplitterKaoOrth(X1,X2,alphaNullSpace,var_cutoff,do_plot)
...
context_switcher = true; % 'true' to start with condition 1, false to start with condition 2
...
for i = 1:(d-1) % loop through dimensions
    ...
    if context_switcher % We're finding an axis for condition 1
        ...
        while flag < 0 % try using defined alphaNullSpace. If it fails, increase and try again
            [Q,flag] = exclusive_subspace(cov(data1),cov(data2),1,alphanull,tol_cutoff); 
            alphanull = alphanull*1.1; % increase by 10%
        end
    else % We're finding an axis for condition 1
        flag = -1; alphanull = alphaNullSpace; 
        while flag < 0
            [Q,flag] = exclusive_subspace(cov(data2),cov(data1),1,alphanull,tol_cutoff); 
            alphanull = alphanull*1.1; 
        end
    end
    context_switcher = ~context_switcher; % switch so we look for other condition next run
...
Qord = [Qf(:,1:2:end) fliplr(Qf(:,2:2:end))]; 
